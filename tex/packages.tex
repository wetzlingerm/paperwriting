...
\usepackage{tocloft}  % for brief contents
...
\usetikzlibrary{arrows,patterns,decorations.pathreplacing}
\usetikzlibrary{external}  % to allow for reading \tikzsetnextfilename, even if \tikzexternalize is not set
% \tikzexternalize[prefix=build/]
% externalization does not work with tcolorbox[enhanced] -> compile in separate file

\usepackage{enumitem}  % lists (currently, modifying items)
\setlist[itemize]{itemsep=0pt, left=8pt}
\newlist{compactitemize}{itemize}{1}
\setlist[compactitemize,1]{nosep, left=8pt, label={\textbullet}}
...
\usepackage{etoolbox}  % for for-loop in newcommand
\usepackage{relsize}  % for resizing font sizes dependent on surrounding size
...
\DeclareFieldFormat{doi}{\mkbibacro{DOI}\addcolon\space\href{https://doi.org/#1}
    {\textsmaller{\textcolor{color_guideline}{\nolinkurl{#1}}}}}
