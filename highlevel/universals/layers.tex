...
\goodexample[{\cite[Sec.~IV.]{Wetzlinger2024CSL}}]{
    ...
    \goodexpl{(Depth 0) The introductory paragraph limits the scope of the section by the terms ``set representations'', ``reachability algorithm'', and ``time complexity'', providing readers with a bounded expectation of what follows.}
    \textit{A. Set Representations and Operations}
    \goodexpl{(Depth 1) The title clearly refers to the first term, ``set representations'', thus limiting the scope of this subsection.}
    ...
    \goodexpl{(Depth 3) The definition is appended a paragraph about set operations.}
    ...
    \goodexpl{(Depth 3) Similary to the second list element, we expand further on this subtopic.}
    \textit{B. Reachability Algorithm}
    \goodexpl{(Depth 1) By structure, we are back at a higher depth, now focusing on the two other terms---``reachability algorithm'' and ``time complexity''---from the introductory sentence to the section.}
...
\noindent
Different readers approach a paper at different depths: some want the full story, others look only for the main ideas, and some search for specific technical details.
