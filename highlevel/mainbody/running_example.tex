...
\goodexample[{Adapted from \cite[Definition~4]{Wetzlinger2023TAC}}]{
    ...
    with
    \begin{equation*}
        G_{\text{keep}} = [G_{(\cdot,\pi_{\chi+1})} \dots G_{(\cdot,\pi_{\gamma})}], \quad
    ...
    Hence, we have the ordering $\pi = [4, 1, 2, 5, 3]$ and $\chi = 3$, which yields
...
\noindent
Include a short example when the notation is hard to follow but becomes clear once understood.
Keep examples concise and only add them if they substantially aid comprehension, as they are optional and may be omitted due to space constraints.
