...
\guideline[g:preliminaries:problemstatement]
    {Introduce all necessary information before the problem statement.}

\badexample[{Adapted from \cite[Sec.~2-3]{Wetzlinger2023HSCC}}]{
    ...
    \end{equation*}
    \badexpl{The highlighted parts are prerequisite definitions, overloading the presentation and distracting from the actual problem statement.}
}
...
\goodexample{
    ...
    \begin{align*}
        ...
        y(t) &= Cx(t) + Wv(t) . \tag{5b}
    \end{align*}
    (...) The initial state $x(0)$, the input $u(t)$, and $v(t)$ are uncertain within the \highlightpart{initial set $\mathcal{X}^0 \subset \mathbb{R}^n$, the input set $\mathcal{U} \subset \mathbb{R}^m$, and the output uncertainty set $\mathcal{V} \subset \mathbb{R}^o$}, respectively. (...) which extends to the \highlightpart{output set $\mathcal{Y}(t)$ obtained from a set-based evaluation of (5b)}. (...)
    \goodexpl{Prerequisite definitions are introduced before the problem statement.}
    \textit{Problem 1. (Verification)}
...
Present all necessary notation and pre-existing knowledge beforehand---either in the preliminaries or immediately above the statement.
