...
\goodbadexample[{\cite[Adapted from Sec.~2~\&~3]{Wetzlinger2023HSCC}}]{
    ...
    ~ Given an LTI system $\dot{x} = Ax(t) + Bu(t)$ where the initial state $x(0)$ and the input $u(t)$ are uncertain within the initial set $\mathcal{X}^0 \subset \mathbb{R}^n$ and the input set $\mathcal{U} \subset \mathbb{R}^m$, respectively, as well as a polytope $\mathcal{K} = \{ z \in \mathbb{R}^r \, | Hz \leq f \}$, decide whether the output set $\mathcal{Y}(t)$ obtained from the output equation $y(t) = Cx(t) + Wv(t)$ where $v(t) \in \mathcal{V} \subset \mathbb{R}^o$ stays within the safe set $\mathcal{K}$ at all times $t \in [0,t_\text{end}]$:
    \begin{equation*}
